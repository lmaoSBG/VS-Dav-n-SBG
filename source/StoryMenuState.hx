...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bgYellow:FlxSprite = new FlxSprite(-900, 56).makeGraphic(FlxG.width, 1000, 0xFF1F52D4);
		var yellowbg2:FlxSprite = new FlxSprite(875, 56).makeGraphic(FlxG.width, 1000, 0xFF1F52D4);
		var blackcover:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 50, FlxColor.WHITE);
		bgSprite = new FlxSprite(0, 56);
		bgSprite.antialiasing = ClientPrefs.globalAntialiasing;

		MiddleBar = new FlxSprite(100, 0).loadGraphic(Paths.image('MiddleBar'));
		MiddleBar.scrollFactor.set(0, 0);
		MiddleBar.setGraphicSize(Std.int(MiddleBar.width * 1));
		MiddleBar.updateHitbox();
		MiddleBar.visible = true;
		MiddleBar.antialiasing = ClientPrefs.globalAntialiasing;
		add(MiddleBar);

		LeftBar = new FlxSprite(-375, 0).loadGraphic(Paths.image('LeftBar'));
		LeftBar.scrollFactor.set(0, 0);
		LeftBar.setGraphicSize(Std.int(LeftBar.width * 1));
		LeftBar.updateHitbox();
		LeftBar.visible = true;
		LeftBar.antialiasing = ClientPrefs.globalAntialiasing;

		RightBar = new FlxSprite(850, 0).loadGraphic(Paths.image('RightBar'));
		RightBar.scrollFactor.set(0, 0);
		RightBar.setGraphicSize(Std.int(RightBar.width * 1));
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 10).loadGraphic(Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		tracksSprite.screenCenter(X);
		add(tracksSprite);

		txtTracklist = new FlxText(FlxG.width * 5, tracksSprite.y + 60, 0, "", 32);
		txtTracklist.alignment = CENTER;
		txtTracklist.font = rankText.font;
		txtTracklist.color = 0xd41f52;
